  ...
  Widget build(BuildContext context) {
    return StoreConnector<AppState, ControlViewModel>(
          converter: (Store<AppState> store) => ControlViewModel.withExecuteFunction(
            () { store.dispatch(BlockUserInputAction(true)); store.dispatch(StopCarAction());} //create dispatch function here, copy this file to every corosponding button (except disabled button.) 
          ),
          builder: (BuildContext context, ControlViewModel vm) => 
           ControlButton(vm: vm, color: _stopColor, icon: Icons.stop,)
          );
  }
